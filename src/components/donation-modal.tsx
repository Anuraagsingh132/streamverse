...
const DonationModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  useEffect(() => {
    // Note: The expiry date in the original code was set to Sep 27, 2025.
    // The current date is Sep 22, 2025, so the countdown will be around 5 days.
  ...
  return (
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/70 backdrop-blur-sm z-50 px-4">
      {/* --- Modal Content --- */}
      <div className="bg-zinc-900 text-white rounded-xl shadow-2xl w-full max-w-md border border-zinc-700/50 relative p-6 sm:p-8 flex flex-col gap-5">
        ...
        <div className="text-center">
          ...
          <div className="flex items-center justify-between bg-zinc-800 p-2 rounded-lg">
            ...
            <button
              ...
              style={{width: '90px'}} // Fixed width to prevent layout shift
            >
              {copyText}
            </button>
          </div>
